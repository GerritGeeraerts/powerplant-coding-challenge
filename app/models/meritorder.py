...
class MeritOrder:
    ...
    def __init__(self, power_plants, desired_load):
        ...
        logger.info(f"Initializing MeritOrder with {len(power_plants)} power plants and desired load of {desired_load} MWh")
    ...
    def set_loads(self):
        ...
        """
        logger.info(f"Setting loads for power plants to meet desired load of {self.desired_load} MWh")
        for index, pp in enumerate(self.power_plants):
            ...
            if self.load + pp.real_pmax <= self.desired_load:
                ...
                logger.debug(f"Set {pp.name} to max output {pp.real_pmax}, total load now {self.load}")
                continue
            if self.load + pp.real_pmin > self.desired_load:
                # minimum of pp is too much, go back and reduce
                logger.warning(f"Minimum output of {pp.name} ({pp.real_pmin}) is too much, current load: {self.load}, desired: {self.desired_load}")
                if index >= 1:
